...
# populate dynamic content for Fact Relationship Prompt
def fact_relationship_prompt_content(file_path):
    """
    Loads the second prompt in the Chain-of-Thought, to identify the confusion underlying the question.
    ...
    # target domain
    file_content = file_content.replace("<insert-target-task>", target_domain.get_task())
    file_content = file_content.replace("<insert-target-facts>", target_domain.get_facts())
...
def fact_relationship_prompt(model_name):
    skeleton_file_path, json_file_path, txt_file_path = get_file_paths(model_name)

    save_to_json(
        fact_relationship_prompt_content(file_path=skeleton_file_path), 
        json_file_path,
        txt_file_path)
...
def main():
    ...
    fact_relationship_prompt("fact_relationship")
